#!/usr/bin/env bash
# Raw memtier JSON staircase benchmark:
#
#   JSON.SET __key__ $ "{\"handle\":\"user\",\"country\":\"USA\",\"timestamp\":1234567890,\"description\":\"hello_from_memtier\"}"
#   JSON.GET __key__
#
# Single `__key__` substitution at position 1 only; the JSON document is
# fixed-content (matches the HASH structured-record shape, expressed as
# JSON). 30-minute staircase: 10 threads × (1→100 clients/thread), +5
# clients/thread every 60s for 20 steps, then 10-min hold at 1000 conns.
#
# Args:
...
# Fixed JSON document — no __key__ inside, so memtier --cluster-mode sees a
# single-key command. Internal " escaped per memtier's --command parser.
JSON_DOC='{\"handle\":\"user\",\"country\":\"USA\",\"timestamp\":1234567890,\"description\":\"hello_from_memtier\"}'
...
echo "Workload : JSON staircase, 50/50 JSON.SET/JSON.GET, 100K keys"
...
memtier_benchmark \
  ...
  --test-time="$TEST_TIME" \
  --command "JSON.SET __key__ \$ \"${JSON_DOC}\"" \
  --command "JSON.GET __key__" \
